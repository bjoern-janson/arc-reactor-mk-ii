...
def _split_score(view: SelectorView, child_mask: int, excluded_query_id: int) -> Fraction:
    ...
    for query_id in view.remaining_query_ids:
        ...
        child0 = candidate_child(child_mask, query_mask, 0)
        child1 = candidate_child(child_mask, query_mask, 1)
        n0 = _count(child0)
        n1 = _count(child1)
...
def feature_vector(view: SelectorView, query_id: int) -> tuple[Fraction, ...]:
    ...
    children = tuple(
        child
        for bit in (0, 1)
        if (child := candidate_child(view.candidate_mask, query_mask, bit)) != 0
    )
    ...
    if view.remaining_budget > 1:
        branch_scores = tuple(_split_score(view, child, query_id) for child in children)
        weighted_next = sum(
